...
인형을 쌓을 stack을 선언한 뒤, moves에 맞춰서 0이 아닌 경우 stack에 쌓는다. 그리고 stack의 개수가 2이상일 때 마지막과 마지막 전 값이 같다면 stack에서 pop을 2번해주고 답을 +2해준다.
...
다른 사람의 풀이를 보면 stack에 다른 값을 넣으면 길이비교가 필요하지 않다고 한다.