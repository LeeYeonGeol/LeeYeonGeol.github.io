...
0 또는 1값을 반환하는 시그모이드의 특성으로 인해 이진 분류의 활률 값을 기반으로 최종 분류 예측을 적용하는데 시그모이드가 사용된다.
...
Sigmoid와 유사하게 Score값을 확률값 0 ~ 1로 변환 하지만, 차이점은 소프트 맥스는 개별 출력값으 ㅣ총 합이 1이 되도록 매핑해주는 것이다.
